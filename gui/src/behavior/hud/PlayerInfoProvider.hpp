...
public:
    void setPlayer(const zappy::PlayerState& player) {
        _name      = "Player " + std::to_string(player.id);
        _team      = player.team;
        _level     = player.level;
        _x         = player.x;
        _y         = player.y;
    ...
    const std::vector<behavior::hud::HudElement>& getHudElements() const override {
        ...
        _cache.push_back({ behavior::hud::TextData{"Team: " + _team,        13.0f, {180, 210, 255, 255}} });
        if (!_votedTeam.empty() && _team == _votedTeam)
            _cache.push_back({ behavior::hud::TextData{"FROM VOTED TEAM", 13.0f, {255, 220, 50, 255}} });
        _cache.push_back({ behavior::hud::TextData{"Level: " + std::to_string(_level), 13.0f, {180, 255, 180, 255}} });
        _cache.push_back({ behavior::hud::TextData{"Pos: (" + std::to_string(_x) + ", " + std::to_string(_y) + ")", 13.0f, {200, 200, 200, 255}} });
        _cache.push_back({ behavior::hud::ButtonData{"Team Chat", 14.0f, 130.0f, 28.0f,
            {255, 255, 255, 255}, {40, 110, 210, 220}, {70, 145, 255, 230},
            _onChatClick} });
        _cache.push_back({ behavior::hud::ButtonData{"Inventory", 14.0f, 130.0f, 28.0f,
