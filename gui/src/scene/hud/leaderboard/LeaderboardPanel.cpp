                              ...
                              graphic::ITextureLoader& loader)
{
    ...
    try {
        ...
        _log.warn("Failed to load leaderboard avatar texture");
    }
    spawnControls(hud);
}
...
void LeaderboardPanel::spawnControls(HudManager& hud)
{
    ...
    float ctrlY = TOP + static_cast<float>(MAX_VISIBLE) * ROW_H + 4.0f;
...
void LeaderboardPanel::onPlayerAdded(uint32_t id, const std::string& team)
{
                                   ...
                                   graphic::Color4b color)
{
    ...
    provider->setTexture(_avatarTex, 64.0f, 64.0f);
    ...
    auto entity = EntityBuilder(hud, id, "leaderboard_entry")
        ...
        .hud().layout(behavior::hud::LayoutEngine::Type::MediaObjectHButtons, 8.0f, 3)
